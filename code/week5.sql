...
CREATE TABLE 학생(
    ...
    나이 INT NULL,
    성별 CHAR(1) NOT NULL,
    휴대폰번호 CHAR(14) NULL, -- 0101-5555-5555
...
CREATE TABLE 수강( 
    ...
    신청날짜 DATE NOT NULL, --2025-04-01
    중간성적 INT NULL DEFAULT 0,
    기말성적 INT NULL DEFAULT 0,
    평가학점 CHAR(1) NULL,
    PRIMARY KEY( 학번, 과목번호) -- s001c001
);
...
SELECT DISTINCT 소속학과 FROM 학생; -- 중복값 제거

SELECT 이름, 학년, 소속학과, 휴대폰번호 FROM 학생
WHERE 학년 >= 4 AND 소속학과 = '컴퓨터';

SELECT 이름, 학년, 소속학과, 휴대폰번호 FROM 학생
WHERE (학년 >= 1 AND 학년 <= 3) OR NOT (소속학과 = '컴퓨터'); --BETWEEN 1 AND 3

SELECT 이름, 학년, 소속학과 FROM 학생
WHERE 소속학과 = '컴퓨터' OR 소속학과 = '정보통신'
ORDER BY 학년 ASC; -- WHERE 소속학과 = '컴퓨터' OR '정보통신' 하면 안됩니다.
...
SELECT * FROM 수강 
ORDER BY 중간성적 DESC
LIMIT 3; -- LIMIT 5,3;은 MYSQL에서만 

SELECT COUNT(*) AS 학생수,
    COUNT(주소) AS 주소수,
    COUNT(DISTINCT 주소) AS 중복되지안은주소 FROM 학생;
...
); -- 과목 번호가 'c002'인 과묵을 수강한 학생의 이름을 검색
...
SELECT 학생.학번, 이름 , 평가학점 -- 오른쪽 외부조인
FROM 학생 RIGHT OUTER JOIN 수강 ON 학생.학번 = 수강.학번;

SELECT 학생.학번, 이름 , 평가학점 -- 전체 외부조인
